...
** File description:
** KinematicMover - API Implementation (Issue #003)
*/
                   ...
                   float deltaTime)
{
    ...
    if (!registry.all_of<Component::RigidBody, Component::RigidBodyInternal>(enttEntity)) {
        Log::Error(fmt::format(
            "[KinematicMover] MoveKinematic: Entity {} has no RigidBody",
    ...
    if (!glm::all(glm::isfinite(targetPosition))) {
        Log::Error(fmt::format(
            "MoveKinematic: targetPosition contains NaN or Inf values ({}, {}, {})",
    ...
    if (deltaTime <= 0.0f) {
        Log::Error(fmt::format(
            "[KinematicMover] MoveKinematic: deltaTime must be positive (got {})",
            deltaTime));
        return;
    }
    ...
    if (rb.motionType != Component::MotionType::Kinematic) {
        Log::Warn(fmt::format(
            "[KinematicMover] MoveKinematic: Body is not kinematic (Entity {}, MotionType: {})",
            entt::to_integral(enttEntity),
    ...
    // Move kinematic body
    physicsManager.GetBodyInterface().MoveKinematic(internal.bodyID, joltTarget,
                                                     joltRotation, deltaTime);
}
                          ...
                          const glm::vec3& velocity, float deltaTime)
{
    ...
    if (!registry.all_of<Object::Component::Transform, Component::RigidBody>(enttEntity)) {
        Log::Error(fmt::format(
            "[KinematicMover] SetKinematicVelocity: Entity {} missing Transform or RigidBody",
            entt::to_integral(enttEntity)));
        return;
    }
    ...
    if (!glm::all(glm::isfinite(velocity))) {
        Log::Error(fmt::format(
            "[KinematicMover] SetKinematicVelocity: Invalid velocity (NaN or Inf)"));
        return;
    }
    ...
    if (deltaTime <= 0.0f) {
        Log::Error(fmt::format(
            "[KinematicMover] SetKinematicVelocity: deltaTime must be positive (got {})",
            deltaTime));
        return;
    }
...
KinematicTarget GetKinematicTarget(Engine::Core& core, Engine::Entity entity)
{
    ...
    JPH::RVec3 joltPosition =
        physicsManager.GetBodyInterface().GetPosition(internal.bodyID);
    JPH::Quat joltRotation =
        physicsManager.GetBodyInterface().GetRotation(internal.bodyID);

    result.position = FromJoltRVec3(joltPosition);
    result.rotation = FromJoltQuat(joltRotation);
