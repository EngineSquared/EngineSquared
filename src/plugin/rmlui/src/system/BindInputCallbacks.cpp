...
void Rmlui::System::BindInputCallbacks(Engine::Core &core)
{
    ...
    callbackIds.mouseButtonCallbackId = input.RegisterMouseButtonCallback([](Engine::Core &context, int button,
                                                                             int action, int mods) {
        context.GetResource<Rmlui::Resource::UIContext>().ProcessMouseButton(button, action, mods);
    });

    callbackIds.cursorPosCallbackId = input.RegisterCursorPosCallback([](Engine::Core &context, double xpos,
                                                                         double ypos) {
        context.GetResource<Rmlui::Resource::UIContext>().ProcessMouseMove(xpos, ypos, 0);
    });

    callbackIds.scrollCallbackId = input.RegisterScrollCallback([](Engine::Core &context, double xoffset,
                                                                   double yoffset) {
