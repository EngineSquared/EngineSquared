...
void ES::Plugin::UI::Utils::EventListener::ProcessMouseButton(int button, int action, int mods)
{
    if (_context)
    {
        switch (action)
        {
        case GLFW_PRESS: _context->ProcessMouseButtonDown(button, ConvertKeyModifiers(mods)); break;
        case GLFW_RELEASE: _context->ProcessMouseButtonUp(button, ConvertKeyModifiers(mods)); break;
        }
    }
}
