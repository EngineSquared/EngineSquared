...
void ES::Plugin::Physics::System::LinkRigidBodiesToPhysicsSystem(entt::registry &registry, entt::entity entity)
{
    ...
    if (shape.HasError())
    {
        ES::Utils::Log::Error(fmt::format("Failed to create shape for entity {}: {}", static_cast<uint32_t>(entity), shape.GetError()));
        return;
    }
    ...
    if (rigidBody.body == nullptr)
    {
        ES::Utils::Log::Error(fmt::format("Failed to create rigid body for entity {}: returned nullptr", static_cast<uint32_t>(entity)));
        return;
    }

    physicsSystem.GetBodyInterface().AddBody(rigidBody.body->GetID(), JPH::EActivation::Activate); // TODO: Not all of them should be activated by default
}
...
{
    core.GetRegistry().on_construct<ES::Plugin::Physics::Component::RigidBody3D>().connect<&ES::Plugin::Physics::System::LinkRigidBodiesToPhysicsSystem>();
    core.GetRegistry().on_destroy<ES::Plugin::Physics::Component::RigidBody3D>().connect<&ES::Plugin::Physics::System::UnlinkRigidBodiesToPhysicsSystem>();
}

void ES::Plugin::Physics::System::SyncTransformsToRigidBodies(ES::Engine::Core &core)
{
    core.GetRegistry().view<ES::Plugin::Physics::Component::RigidBody3D, ES::Plugin::Object::Component::Transform>().each([&](auto &rigidBody, auto &transform) {
        if (rigidBody.body != nullptr)
        {
...
void ES::Plugin::Physics::System::PhysicsUpdate(ES::Engine::Core &core)
{
    ...
    auto &physicsManager = core.GetResource<ES::Plugin::Physics::Resource::PhysicsManager>();
    physicsManager.GetPhysicsSystem().Update(dt, physicsManager.GetCollisionSteps(), physicsManager.GetTempAllocator(), physicsManager.GetJobSystem());
}