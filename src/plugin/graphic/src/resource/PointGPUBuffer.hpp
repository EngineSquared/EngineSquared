...
namespace Graphic::Resource
{
    ...
    public:
        ...
        void Create(Engine::Core &core) {
            ...
            if (!meshComponent) {
                throw Exception::NonexistentComponentError("Cannot create a GPU buffer for an entity without a Mesh component.");
            }
        ...
        bool IsCreated(Engine::Core &core) const {
            return _isCreated;
        };
        void Update(Engine::Core &core) {
            ...
            // For now, we will not implement dynamic resizing of the buffer. As we should have a way to know if the size changed. And it would be so heavy to check every frame every vertex position, normal and texCoord.
        };
