    ...
    public:
        ...
        inline void RegisterSound(const std::string &soundName, const std::string &filePath, bool loop = false) {
            ...
            if (result != MA_SUCCESS) {
                ES::Utils::Log::Error(fmt::format("Failed to initialize the audio device: {}", ma_result_description(_result)));
                return;
            }
        ...
        inline void SetLoopPoints(const std::string &soundName, float startSeconds, float endSeconds = 0)
        {
            ...
            if (it != _soundsToPlay.end()) {
                ...
                if (ma_decoder_get_length_in_pcm_frames(&decoder, &totalFrames) != MA_SUCCESS) {
                    ES::Utils::Log::Error(fmt::format("Something went wrong while computing PCM frames length of sound \"{}\"", soundName));
                    return;
                }
                ...
                if (startFrame >= totalFrames || endFrame > totalFrames || startFrame >= endFrame) {
                    ES::Utils::Log::Error(fmt::format("Invalid loop range for \"{}\": {}s to {}s", soundName, startSeconds, endSeconds));
                    return;
                }
...
} // namespace ES::Plgun::Sound::Resource
