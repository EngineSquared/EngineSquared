...
void Graphic::System::ExecuteRenderPass(Engine::Core &core)
{
    ...
    core.GetRegistry().view<Object::Component::Camera>().each([&renderPassContainer,
                                                               &core](auto e, const Object::Component::Camera &camera) {
        if (!renderPassContainer.Contains(camera.pipeline))
        {
            Engine::Entity entity(e);
            Log::Error(
                fmt::format("ExecuteRenderPass: Camera with Pipeline '{}' not found for entity {} camera component.",
