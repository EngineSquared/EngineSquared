...
void ShutdownJoltPhysics(ES::Engine::Core &core)
{
    ...
    core.GetRegistry().view<ES::Plugin::Physics::Component::SoftBody3D>().each([&](auto entity, const auto &) {
        core.GetRegistry().remove<ES::Plugin::Physics::Component::SoftBody3D>(entity);
    });

    core.GetRegistry().view<ES::Plugin::Physics::Component::WheeledVehicle3D>().each([&](auto entity, const auto &) {
