...
namespace Graphic::Resource
{
    ...
    public:
        ...
        void WriteTexture(Context &context, const ImageData &imageData) {
            ...
            if (imageData.width != this->_webgpuTexture.getWidth() ||
                imageData.height != this->_webgpuTexture.getHeight()) {
            ...
            queue.writeTexture(destination, imageData.pixels, source.bytesPerRow * source.rowsPerImage, source, textureSize);
        }
    ...
    private:
        uint32_t getBytesPerPixel() const {
            switch (_webgpuTexture.getFormat()) {
                ...
                default:
                    throw std::runtime_error("Unsupported texture format for byte size calculation.");
            }
        }
