...
struct EntityId : Id {
    ...
    template <typename TComponent> decltype(auto) AddComponent(Engine::Core &core, TComponent &&component);
    ...
    template <typename... TComponent> bool HasComponents(const Engine::Core &core) const;
    ...
    template <typename... TComponent> decltype(auto) GetComponents(Engine::Core &core);

    /// @brief Get components of type TComponent from the entity.
    /// @tparam TComponent The type of the component to get from the registry.
    /// @param core The Core instance whose registry is used to store the component.
    /// @return The components of type TComponent from the entity.
    template <typename... TComponent> decltype(auto) GetComponents(Engine::Core &core) const;
    ...
    /// @param core The Core instance whose registry is used to store the component.
    template <typename TComponent> void RemoveComponent(Engine::Core &core);
