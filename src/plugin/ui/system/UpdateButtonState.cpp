...
void ES::Plugin::UI::System::UpdateButtonState(ES::Engine::Registry &r) {
	...
	auto view = r.GetRegistry().view<ES::Plugin::UI::Component::Button, ES::Plugin::Collision::Component::BoxCollider2D, ES::Plugin::Object::Component::Transform>();
	for (auto entity : view) {
		...
		const ES::Plugin::Math::Rect &rect = { {transform.position.x, transform.position.y}, {collider.size.x * transform.getScale().x, collider.size.y * transform.getScale().y} };
		...
		if (button.lastState != button.state && !ES::Engine::Entity(entity).HasComponents<ES::Plugin::Tools::HasChanged<ES::Plugin::UI::Component::Button>>(r)) {
			ES::Engine::Entity(entity).AddTemporaryComponent<ES::Plugin::Tools::HasChanged<ES::Plugin::UI::Component::Button>>(r);
		}
	}
}